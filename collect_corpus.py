...
#   · + check_seeds.py 가 공식 명단(api.nobelprize.org)과 대조해 찾은 분류 누락 1명 (2025년 수상자)
# 명단을 바꿀 때는 check_seeds.py 로 먼저 차이를 보고, 사람이 확인한 뒤 여기를 고친다.
SEEDS = [
...
def _fetch(params, url=API):
    """429·5xx·연결 오류는 기다렸다 다시 친다. check_seeds.py 도 위키데이터 호출에 이것을 쓴다."""
