...
def test_zones_respect_switch():
    ...
    # 기본(수업 방식) — 남의 시작문서 + 읽은 문서만. 공용 서가(노벨 문학상 · 노벨상)는 피하지 않는다
    assert graph.zones(toc, done, 0, SW) == ["가와바타 야스나리", "설국"]
    # 담당구역을 켜면 남의 담당문서도 피한다
    assert graph.zones(toc, done, 0, {**SW, "담당구역": True}) == ["가와바타 야스나리", "설국", "오에 겐자부로"]
