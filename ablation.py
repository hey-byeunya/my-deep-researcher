...
결과: output/runs.jsonl (실행마다 한 줄, '실험'·'반복' 표시) · output/ablation.json (요약)
"""
import argparse
import json
import statistics
...
OUT = graph.OUTPUT / "ablation.json"
...
def summarize(exp):
    ...
    OUT.write_text(json.dumps({"실험": exp, "만든시각": time.strftime("%Y-%m-%d %H:%M"),
                               "주의": LEGACY.get(exp, ""),
