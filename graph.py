...
SWITCHES = {
    ...
    "기획보강": True, # 끄면 제출본(ablation-2)의 기획으로 돌아간다. 켜면 코드 쪽 검사만 더한다 — 시대 절 연도 검사
                      ...
                      # 효과는 켬/끔을 같은 시간대에 번갈아 잰다(기획만 재는 개발용 스크립트 · ablation-3) — 따로 잰 묶음끼리는 모델
...
# 수상 연도 — 노벨상 공식 API 에서 온 값 (명단 대조 스크립트가 data/award_years.json 에 남겼다 — 개발용이라 저장소에는 없다)
...
def cards(width=None):
    ...
    카드마다 번호(D17)를 붙이고 "번호 제목"으로 배정받는 방식도 시험했다(기획만 번갈아 잰 개발 기록 'P1a-번호카드').
...
def plan(s):
    ...
    if problems:
        ...
        # 직전 목차를 함께 돌려주는 방식도 시험했다(기획만 번갈아 잰 개발 기록 'P2-번갈아'). 재기획이 나아진 횟수는 늘지 않았고,
