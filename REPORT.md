...
> 제출 뒤에 한 기획 강화(§5 끝)는 `output/ablation-3.json`(30번) · `output/plans.jsonl`(기획만 잰 기록)에서 왔다.
...
**재는 방법부터.** 기획만 따로 재는 `plan_check.py`(한 번에 LLM 1\~2회)를 만들어 Q2 · Q5 · Q8 × 5회씩 쟀다. 그러다
...
재고(`--settings 켬 기획보강끔`), 5번 중 서로 다른 목차가 몇 개였는지도 적는다(1이면 5번이 사실상 표본 하나). 같은 설정인
...
- **비교형 질문** — 사람별 절 각각에 "상대와 이어지는 점 · 갈라지는 점"을 지시한다(Q8 추적 ③). 지시문으로 시켜 보니 따르지
  않아서, 빈 비교 절의 지시를 코드가 넘겨주는 방식으로 옮겼다(§5 끝) — Q8 은 사람의 판단으로도 나아졌다.
...
| **엔드투엔드 구동 및 제출물의 완결성** — 자료 수집부터 장문 보고서 생성까지 전체 워크플로우가 오류 없이 실행되는가? | README — `check_seeds.py` → `collect_corpus.py` → `graph.py` → `baseline.py` → `ablation.py` → `app.py`. 72번 실험이 모두 끝까지 돌아 기록됐고(제출 뒤 ablation-3 30번도), 테스트 97개(가짜 모델로 전 구간), 데모의 라이브 실행을 브라우저에서 확인 |
