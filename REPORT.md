...
4. **수상 연도는 노벨상 공식 API**(`check_seeds.py` → `data/award_years.json`). 공식 데이터 쪽 오류도 하나 찾았다 —
...
flowchart TD
    ...
    subgraph 코디네이터
        ...
        check{"validate_plan · 코드 검사<br/>실재 · 겹침 · 형식 단위 · 역할"}
...
| 칸 | 리듀서 | 누가 쓰나 | 내용 |
|---|---|---|---|
| `question` · `switches` | — | 시작 | 질문, 실험 스위치 6개 |
...
`streamlit run app.py`. 화면 캡처는 `docs/screenshots/`(`make_screenshots.py` 로 다시 만든다).
...
- **무엇을 할까요** — ① 질문하기(직접 쓰고 Enter, 기본) ② 데모 보기(저장된 실행 139건을 **API 키 없이**) ③ 질문 선택(준비한 10건).
...
5. **재현성.** 코퍼스 고정(`--refresh`에서만 다시 수집), 실행마다 스위치 · 설정 · 코퍼스 버전(해시) 저장, 지표는 기록만으로 재계산,
   테스트 87개는 가짜 모델로 네트워크 없이 돈다.

### 알려진 한계
- **기획이 가장 불안정하다** — 팀 기본 9회 중 8회에서 재기획이 일어났고, 같은 질문도 시행마다 목차가 달랐다(보완안은 아래 '기획 안정화').
- 실험은 설정당 3회뿐이라 5점 안팎의 차이는 가를 수 없다(역할 · 재위임).
- 한국어 위키백과에 작품 문서가 적어 작품이 있는 작가는 28명뿐이다.
- 수업 방식 격리율은 예산 크기에 흔들려, 격리는 카드 열람률과 절간 중복으로 따로 보여야 한다.

### 향후 보완하고 싶은 점

**기획 안정화 — 가장 먼저 할 일.** 제출본은 ablation-2 조건 그대로 두고 방안만 적는다(적용하면 ablation-3 으로 다시 잰다).

ablation-2 에서 배정이 켜진 팀 실행 45번을 세면 다음과 같다.

| 증상 | 숫자 | 뜻 |
|---|---|---|
| 재기획 발동 · 실제로 나아짐 | 38번 · 그중 17번 | 첫 목차는 거의 매번 문제가 있고, 재기획은 절반 넘게 소용이 없다 |
| 빈 절을 빼고 파견 | 21번 | 겹친 문서를 뒤쪽 절에서 빼다 보니 절이 통째로 빈다 |
| 절 수 | 1개 1 · 2개 15 · 3개 21 · 4개 8번 | 같은 질문도 절 수가 달라진다 |
| 시대 절에 기간 밖 수상자 | 138건 중 21건 | 거의 전부 「중기 (1951-2000)」에 옐리네크(2004) · 레싱(2007) |
| 카드 줄을 연도째 베낌 · 코퍼스에 없는 제목 | 62건 · 23건 | 코드가 고치거나 뺐다 |

원인은 셋으로 본다.
1. **모델이 한 번에 정하는 것이 너무 많다** — 나누는 축 · 절 제목 · 문서 배정 · 비중을 한 번에 정한다.
2. **재기획이 막연한 재도전이다** — 코드가 문제 목록은 주지만 자기 목차는 돌려주지 않아, 백지에서 다시 짜다 다른 데서 또 겹친다.
3. **역할 이름이 형식 단위 목차를 부른다** — Q5 1회차 목차가 「생애와 시대적 배경 / 작품과 주제 / 사상과 문학적 흐름 /
   시대적 변화」로 역할 명단(생애 · 작품 · 사상 · 시대)을 그대로 옮겼다. 절 제목이 정확히 '배경'일 때만 잡는 지금 검사로는 통과한다.

방안은 비용이 적은 순서로 적는다. ①~⑤는 LLM 호출과 격리 비용을 늘리지 않는다.

| 단계 | 방안 | 겨냥하는 증상 |
|---|---|---|
| 코드 | ① 절 제목에 기간이 있으면 공식 수상 연도로 담당 수상자를 검사해 맞는 시대 절로 옮기고, 새 경보 '시대불일치'로 센다 | 기간 밖 21건 |
| 코드 | ② 겹친 문서를 '뒤 절에서 빼기' 대신 제목 · 지시와 가장 맞는 절, 또는 담당문서가 적은 절에 나눠 준다 | 빈 절 21번 |
| 코드 | ③ 카드에 번호(`[D17]`)를 붙이고 번호로 배정받는다 — «»를 데이터로 바꿔 근거율 0%를 고친 것과 같은 원리 | 베낌 62건 · 없는 제목 23건 |
| 재기획 | ④ 직전 목차와 "어느 절의 어느 문서가 어느 절과 겹쳤는지"를 함께 돌려줘 고쳐 쓰게 한다 | 재기획 무효 21번 |
| 프롬프트 · 검사 | ⑤ "역할은 읽는 관점이지 나누는 기준이 아니다"를 넣고, 역할 이름으로만 된 절 제목도 형식 단위로 잡는다 | 역할명 목차 |
| 구조 | ⑥ 2단계 기획 — 1차에 축(사람 · 나라 · 시대 · 작품 · 쟁점)과 절 제목만, 2차에 배정(시대 축이면 코드가 연도로). 호출 1번과 카드 열람이 는다(2차는 제목 + 연도만 담은 짧은 카드로 줄인다) | 결정이 한 번에 몰림 |
| 구조 | ⑦ 목차 후보 2~3개 중 코드 점수가 가장 좋은 것 — 가장 확실하지만 카드를 여러 번 보내 격리 숫자가 나빠져, 이 프로젝트의 요점과 부딪친다 | 전체 |

OpenAI `seed` 로 결과를 고정하는 방법은 흔들림을 고치지 않고 가리므로 쓰지 않는다 — 흔들림 자체가 보고할 거리다.
효과는 전 구간을 다시 돌리지 않고 `graph.py --plan-only` 로 Q2 · Q5 · Q8 × 5회(약 30번 호출)만 재서 본다 — 재기획 발동 · 채택률,
빈 절 제외, 시대불일치, 배정 실패, 같은 질문 5회의 담당문서 겹침(목차 일관성). 나아졌을 때만 전 구간 실험으로 넘어간다.
...
- **비교형 질문** — 사람별 절 각각에 "상대와 이어지는 점 · 갈라지는 점"을 지시한다(Q8 추적 ③).
...
- 코드 리뷰에서 넘긴 항목: 실행 파일명의 초 단위 충돌 대비, `_llm` 동시 호출과 예외 매칭, CSS 토큰 단일 소스화,
  문장 분리 규칙의 한계 명시.
- 반복 수를 늘려(5회 이상) 역할 · 재위임의 효과를 다시 잰다.
...
| 루브릭 | 이 보고서의 증거 |
|---|---|
| **성능 검증 및 실패 분석** — 설정을 바꿔 만든 보고서 여러 편을 나란히 읽고, 어느 쪽이 나은지와 그 이유를 자기 말로 적었는가? 마음에 안 드는 대목을 어느 절 · 어느 자료까지 추적했는가? | §5 — Q5 · Q8 여섯 쌍을 사람이 읽은 판단과 이유, 마음에 안 드는 대목 세 건을 절 · 자료 · 장치까지 추적(구르나 혼입 · 반복 편집 · 뭉툭한 맺음말), 절제 실험 8설정 × 3회 |
...
| **엔드투엔드 구동 및 제출물의 완결성** — 자료 수집부터 장문 보고서 생성까지 전체 워크플로우가 오류 없이 실행되는가? | README — `check_seeds.py` → `collect_corpus.py` → `graph.py` → `baseline.py` → `ablation.py` → `app.py`. 72번 실험이 모두 끝까지 돌아 기록됐고, 테스트 87개(가짜 모델로 전 구간), 데모의 라이브 실행을 브라우저에서 확인 |
...
| 3. 무엇을 보고 판단할지 | 정답표 · 판정 모델 없는 지표 19종, 지표마다 보는 장치, 신호/경보 구분 |
